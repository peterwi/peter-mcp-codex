 ...
 */
async function saveToolState(tool, state) {
    try {
 ...
 */
async function checkKernelHeaders() {
 ...
 */
export async function bccPreflight(tool) {
    ...
    const toolAvailable = caps.bccTools[tool] ?? false;
    if (!toolAvailable) {
 ...
 */
export function calculateTimeout(config, caps) {
 ...
 */
export async function executeBcc(config) {
 ...
 */
async function executeBpftraceFallback(config, startTime, warnings, _preflight) {
    if (!config.bpftraceFallback) {
        return {
            success: false,
 ...
 */
export async function warmupBccTool(tool) {
    const startTime = Date.now();
...
//# sourceMappingURL=bcc-runtime.js.map