 ...
 */
export function getSyscountScript(durationSec, pid) {
 ...
 */
export function getDnsLatencyScript(durationSec, pid) {
    const pidFilter = pid ? `pid == ${pid}` : '1';
    return `
 ...
 */
export function getBioLatencyScript(durationSec, _device) {
 ...
 */
export function getProcessTraceScript(durationSec, includeFork = true, includeExec = true, includeExit = false, pid) {
 ...
 */
export function getFileOpsScript(durationSec, minLatencyMs = 10, pid) {
    const pidFilter = pid ? `pid == ${pid}` : '1';
 ...
 */
export function getRunqlatScript(durationSec, _pid) {
 ...
 */
export function getOffcpuScript(durationSec, _pid, minBlockUs = 1000) {
    // Pid filtering would need to be added to the script if needed
    return `
tracepoint:sched:sched_switch
{
 ...
 */
export function getTcpTraceScript(durationSec, pid) {
    const pidFilter = pid ? `pid == ${pid}` : '1';
    return `
 ...
 */
export function getFallbackScript(tool, durationSec, options) {
    switch (tool) {
        case 'syscount':
        ...
        case 'execsnoop':
            return getProcessTraceScript(durationSec, options?.includeFork ?? true, options?.includeExec ?? true, options?.includeExit ?? false, options?.pid);
        case 'fileslower':
...
//# sourceMappingURL=bpftrace-fallbacks.js.map