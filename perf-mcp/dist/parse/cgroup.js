 ...
 */
export function parseCgroupCpuStat(content) {
    const values = {};
 ...
 */
export function parseCgroupCpuMax(content) {
 ...
 * ...
 */
export function parseCgroupMemoryStat(content) {
    const values = {};
    for (const line of content.split('\n')) {
        const parts = line.split(' ');
        if (parts.length === 2) {
 ...
 */
export function parseCgroupMemoryCurrent(content) {
 ...
 */
export function parseCgroupMemoryMax(content) {
 ...
 * oom_group_kill 0
 */
export function parseCgroupMemoryEvents(content) {
    const values = {};
    for (const line of content.split('\n')) {
        const parts = line.split(' ');
        if (parts.length === 2) {
 ...
 */
export function parseCgroupIoStat(content) {
    const devices = [];
    for (const line of content.split('\n')) {
        if (!line.trim())
            continue;
 ...
 */
export function parseCgroupPidsCurrent(content) {
    return parseInt(content.trim(), 10) || 0;
}
/**
 ...
 * Returns -1 for "max" (no limit)
 */
export function parseCgroupPidsMax(content) {
    const trimmed = content.trim();
    if (trimmed === 'max') {
        return -1;
    }
 ...
 */
export function buildCgroupMemoryStats(current, max, stat, events) {
 ...
 */
export function parseCgroupPressure(content) {
    const result = {
        some_avg10: 0,
        some_avg60: 0,
        some_avg300: 0,
    ...
    for (const line of content.split('\n')) {
        ...
        if (type !== 'some' && type !== 'full')
            continue;
        for (const part of parts.slice(1)) {
            const [key, value] = part.split('=');
            const numValue = parseFloat(value);
            if (key === 'avg10') {
                result[`${type}_avg10`] = numValue;
            }
            else if (key === 'avg60') {
                result[`${type}_avg60`] = numValue;
            }
            else if (key === 'avg300') {
                result[`${type}_avg300`] = numValue;
            }
            else if (key === 'total') {
                result[`${type}_total`] = numValue;
            }
        }
    }
    return result;
 ...
 */
export function identifyCgroupIssues(cpuStats, _cpuMax, memStats, memPressure) {
    const issues = [];
    // CPU throttling
    if (cpuStats.nr_throttled > 0) {
        const throttledPercent = cpuStats.usage_usec > 0
            ? (cpuStats.throttled_usec / cpuStats.usage_usec) * 100
            : 0;
        if (throttledPercent > 5) {
            issues.push(`CPU severely throttled: ${cpuStats.nr_throttled} times, ` +
    ...
    if (memStats.usage_percent > 90) {
        issues.push(`Memory at ${memStats.usage_percent.toFixed(1)}% of limit ` +
