 ...
 */
export function parsePerfReport(output) {
    const samples = [];
 ...
 */
export function parsePerfSchedLatency(output) {
    const tasks = [];
    const lines = output.split('\n');
    for (const line of lines) {
        ...
        const match = line.match(/^\s*([^|]+?):(\d+)\s*\|\s*([\d.]+)\s*ms\s*\|\s*(\d+)\s*\|\s*avg:\s*([\d.]+)\s*ms\s*\|\s*max:\s*([\d.]+)\s*ms/);
        if (match) {
            tasks.push({
                task: match[1].trim(),
...
export function parsePerfSchedTimehist(output) {
    const entries = [];
    const lines = output.split('\n');
    for (const line of lines) {
        // Skip header/separator lines
        ...
        if (!line.trim())
            continue;
        // Parse entry line
        const match = line.match(/^\s*([\d.]+)\s+\[(\d+)\]\s+(\S+)\[(\d+)\]\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);
        if (match) {
            entries.push({
 ...
 */
export function aggregateTimehist(entries) {
 ...
 */
export function parseCollapsedStacks(output) {
 ...
 */
export function calculateKernelUserSplit(samples) {
    let kernel = 0;
    let user = 0;
    for (const sample of samples) {
        if (sample.module.includes('kernel') ||
