...
export function parseProcStat(content) {
    ...
    const [user = 0, nice = 0, system = 0, idle = 0, iowait = 0, irq = 0, softirq = 0, steal = 0] = cpuParts;
 ...
 * Parse /proc/loadavg
 */
export function parseProcLoadavg(content) {
 ...
 * Parse /proc/meminfo
 */
export function parseProcMeminfo(content) {
    const values = {};
 ...
 */
export function parseProcVmstat(content) {
    const values = {};
    for (const line of content.split('\n')) {
 ...
 * Parse /proc/uptime
 */
export function parseProcUptime(content) {
    const parts = content.trim().split(/\s+/);
    return {
 ...
 */
export function parseProcNetSnmp(content) {
    const lines = content.split('\n');
    let tcpHeader = [];
    let tcpValues = [];
 ...
 */
export function parseProcPressure(content) {
    const result = {
        some_avg10: 0,
        some_avg60: 0,
        some_avg300: 0,
 ...
 */
export function parseProcPidCgroup(content) {
 ...
 */
export function parseProcCpuinfo(content) {
    let model = 'Unknown';
    let cores = 0;
    let threads = 0;
...
export function parseProcDiskstats(content) {
    const stats = [];
    for (const line of content.split('\n')) {
