...
const MAX_FILES = 200;
const MAX_OPENS = 300;
export async function perfFileTrace(input = {}) {
    ...
    try {
        ...
        if (needsFileslower && hasFileslower) {
            ...
            if (result.stdout) {
                ...
                let operations = parsed.operations.map((op) => ({
        ...
        if (needsOpensnoop && hasOpensnoop) {
            ...
            if (result.stdout) {
                ...
                let operations = parsed.opens.map((o) => ({
        ...
        if (params.min_latency_ms > 0)
            filters.min_latency_ms = params.min_latency_ms;
        return {
            success: true,
            tool: 'perf_file_trace',
            tool_version: TOOL_VERSION,
