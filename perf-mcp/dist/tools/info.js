...
export async function perfInfo() {
    ...
    try {
        ...
        try {
            const governorResult = await safeReadFile('/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor');
        ...
        if (!caps.hasPerf) {
            ...
            warnings.push(`perf access restricted (paranoid=${caps.perfEventParanoid}) - run as root or adjust setting`);
        }
        if (!caps.hasPsi) {
