...
export async function perfNetHealth(options = {}) {
    ...
    try {
        ...
        if (filterInterface) {
            interfaces = interfaces.filter((iface) => iface.name === filterInterface || iface.name.startsWith(filterInterface));
        }
        ...
        const retransmitRate = tcpStats.out_segs > 0 ? (tcpStats.retransmits / tcpStats.out_segs) * 100 : 0;
        ...
        const enhancedInterfaces = interfaces.map((iface) => ({
            ...iface,
            speed_mbps: undefined, // Would need ethtool
            duplex: undefined,
        }));
        const data = {
...
//# sourceMappingURL=net-health.js.map