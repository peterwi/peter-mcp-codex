...
export async function perfTcpTrace(input = {}) {
...
async function runTcplife(params, startTime, notes) {
    const args = ['-T']; // Include timestamps
    if (params.pid) {
...
async function runTcpconnect(params, startTime, notes) {
    const args = ['-t']; // Include timestamps
    if (params.pid) {
        args.push('-p', String(params.pid));
    }
...
//# sourceMappingURL=tcp-trace.js.map