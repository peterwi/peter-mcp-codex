 ...
 */
function parsePidstatThreads(output) {
    const lines = output.trim().split('\n');
    const entries = [];
    for (const line of lines) {
        // Skip non-average lines and headers
        if (!line.startsWith('Average:') || line.includes('%usr') || line.includes('UID'))
            continue;
 ...
 */
async function getThreadState(pid, tid) {
 ...
 */
async function getThreadContextSwitches(pid, tid) {
...
export async function perfThreadProfile(input) {
...
//# sourceMappingURL=thread-profile.js.map