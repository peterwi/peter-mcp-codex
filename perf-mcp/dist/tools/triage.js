 ...
 */
function analyzeRootCauses(findings, _evidence, _toolResults) {
    const hypotheses = [];
    ...
    if (criticalMem.length > 0) {
        ...
        if (lowMemory || swapUsage) {
            hypotheses.push({
                ...
                supportingFindings: [lowMemory?.id, swapUsage?.id].filter(Boolean),
                suggestedActions: [
 ...
 */
function generateExecutiveSummary(hypotheses, findings, target) {
    const parts = [];
    // Target description
    if (target.pid) {
 ...
 */
async function runTool(name, fn, extractFindings, extractEvidence) {
...
export async function perfTriage(input = {}) {
    const startTime = Date.now();
    try {
        ...
        const snapshotResult = await runTool('perf_snapshot', async () => {
            ...
            if (cpuUtil > 90) {
                findings.push(createFinding('high_cpu_utilization', 'warning', 'High CPU utilization', `CPU utilization is ${cpuUtil.toFixed(1)}%`, 'cpu', { metrics: { utilization: cpuUtil } }));
            }
            if (availableMb < 500) {
                findings.push(createFinding('low_available_memory', 'critical', 'Low available memory', `Only ${availableMb.toFixed(0)}MB available`, 'memory', { metrics: { available_mb: availableMb } }));
            }
            return findings;
        }, (data) => {
            const cpuUtil = 100 - data.cpu.utilization.idle;
        ...
        // 2. USE check
        const useResult = await runTool('perf_use_check', () => perfUseCheck(), (data) => {
            const findings = [];
            // Check top suspicions from summary
            for (const suspicion of data.summary.top_suspicions) {
                findings.push(createFinding('use_suspicion', data.summary.status === 'critical' ? 'critical' : 'warning', 'USE method finding', suspicion, 'system'));
            }
            ...
            for (const [resource, metrics] of Object.entries(data.resources)) {
                if (metrics.utilization.status === 'critical' || metrics.utilization.status === 'warning') {
                    findings.push(createFinding(`use_${resource}_utilization`, metrics.utilization.status === 'critical' ? 'critical' : 'warning', `${resource} utilization issue`, metrics.utilization.detail, resource));
                }
                if (metrics.saturation.status === 'critical' || metrics.saturation.status === 'warning') {
                    findings.push(createFinding(`use_${resource}_saturation`, metrics.saturation.status === 'critical' ? 'critical' : 'warning', `${resource} saturation issue`, metrics.saturation.detail, resource));
                }
                if (metrics.errors.status === 'critical' || metrics.errors.status === 'warning') {
                    findings.push(createFinding(`use_${resource}_errors`, metrics.errors.status === 'critical' ? 'critical' : 'warning', `${resource} errors detected`, metrics.errors.detail, resource));
                }
            }
            return findings;
        });
        ...
        if (params.mode !== 'quick') {
            ...
            if (caps.canUseBpf || caps.hasBpftrace) {
                const syscallResult = await runTool('perf_syscall_count', () => perfSyscallCount({
            ...
            // Thread profile
            if (params.pid) {
                const threadResult = await runTool('perf_thread_profile', () => perfThreadProfile({
                    pid: params.pid,
            ...
            if (caps.canUseBpf) {
                const ioResult = await runTool('perf_io_layers', () => perfIoLayers({
                    duration_seconds: Math.min(durationSec, 10),
                }));
        ...
        if (params.mode === 'deep') {
            // File trace
            if (caps.canUseBpf) {
                const fileResult = await runTool('perf_file_trace', () => perfFileTrace({
            ...
            if (params.include_exec_trace && (caps.canUseBpf || caps.hasBpftrace)) {
                const execResult = await runTool('perf_exec_trace', () => perfExecTrace({
                    duration_seconds: Math.min(durationSec, 15),
                    pid: params.pid,
        ...
        if (snapshotResult.success && rawResults.snapshot) {
            const snap = rawResults.snapshot.data;
