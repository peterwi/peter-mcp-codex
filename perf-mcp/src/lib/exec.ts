 ...
 */
function validateArgs(command: AllowedCommand, args: string[]): boolean {
  for (const arg of args) {
    ...
    if (arg.startsWith('-') || command.allowedArgs.includes(arg)) {
      ...
      const isAllowed = command.allowedArgs.some(
        (allowed) => arg === allowed || arg.startsWith(allowed + '=')
      );
...
export async function safeReadFile(
...
): Promise<{ success: true; content: string } | ExecError> {
  ...
  try {
    ...
    const nodeError = caughtErr as NodeJS.ErrnoException;
 ...
 */
export function isExecError(result: ExecResult | ExecError): result is ExecError {
