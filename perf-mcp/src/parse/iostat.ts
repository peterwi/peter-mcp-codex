 ...
 */
export function parseIostat(output: string): IoDeviceStats[] {
  const devices: IoDeviceStats[] = [];
 ...
 */
export function parseIostatCpu(
  output: string
): { user: number; system: number; iowait: number; idle: number } | null {
  const lines = output.split('\n');
