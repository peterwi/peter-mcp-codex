...
export interface BioLatencyData {
  ...
  bucket_size_ms?: number; // Only for linear mode
 ...
 */
function buildLinearBpftraceScript(bucketMs: number, durationSec: number): string {
...
export async function perfBioLatency(
  input: BioLatencyRawInput = {}
): Promise<PerfResponse<BioLatencyData>> {
  ...
  try {
    ...
    let perDeviceSummary: Record<string, { total_ios: number; avg_latency_us: number }> | undefined;
