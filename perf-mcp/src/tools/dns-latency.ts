...
export interface DnsLatencyData {
  ...
  summary: {
    ...
    by_host: Record<string, { count: number; avg_latency_ms: number }>;
...
export async function perfDnsLatency(
  input: DnsLatencyRawInput = {}
): Promise<PerfResponse<DnsLatencyData>> {
  ...
  try {
    ...
    let queries: DnsQuery[] = parsed.lookups.map((l) => ({
    ...
    const processStats: Record<string, { count: number; totalMs: number }> = {};
    ...
    const byHost: Record<string, { count: number; avg_latency_ms: number }> = {};
