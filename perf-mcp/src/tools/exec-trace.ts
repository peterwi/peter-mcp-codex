...
// Legacy exec event for backwards compatibility
...
export interface ExecTraceData {
  ...
  // New comprehensive event format
  process_events?: ProcessEvent[];
  relationships?: ProcessRelationship[];
  tree?: ProcessTreeNode[];
  // Legacy events format (when using execsnoop only)
  events?: ExecEvent[];
  summary: {
    total_execs: number;
 ...
 */
function buildProcessTraceScript(
  durationSec: number,
  includeFork: boolean,
  includeExec: boolean,
  includeExit: boolean,
 ...
 */
function parseBpftraceOutput(output: string): ProcessEvent[] {
  const events: ProcessEvent[] = [];
  const lines = output.trim().split('\n');

  for (const line of lines) {
    // Skip non-data lines
    if (line.startsWith('Attaching') || line.startsWith('@') || !line.trim()) continue;
 ...
 */
function buildRelationships(events: ProcessEvent[]): ProcessRelationship[] {
 ...
 */
function buildProcessTree(events: ProcessEvent[]): ProcessTreeNode[] {
 ...
 */
function getMaxTreeDepth(nodes: ProcessTreeNode[], depth = 1): number {
  let maxDepth = depth;
...
export async function perfExecTrace(
  input: ExecTraceRawInput = {}
): Promise<PerfResponse<ExecTraceData>> {
 ...
 */
async function runBpftraceComprehensive(
  params: ExecTraceInput,
  _caps: Awaited<ReturnType<typeof detectCapabilities>>,
  notes: string[],
  startTime: number
): Promise<PerfResponse<ExecTraceData>> {
  notes.push('Using bpftrace for comprehensive process tracing');

  const script = buildProcessTraceScript(
    params.duration_seconds,
    params.include_fork_clone,
    params.include_exec,
    params.include_exit,
    params.pid
  );
  ...
  if (params.name_pattern) {
    ...
    processEvents = processEvents.filter(
      (e) =>
        e.comm.toLowerCase().includes(pattern) ||
  ...
  let tree: ProcessTreeNode[] | undefined;
  let maxTreeDepth: number | undefined;
  ...
  const topCommands = Object.fromEntries(
    Object.entries(byCommand).sort((a, b) => b[1] - a[1]).slice(0, 20)
  );
  const topParents = Object.fromEntries(
    Object.entries(byParent).sort((a, b) => b[1] - a[1]).slice(0, 20)
  );
  ...
  // Build filter info
  const filters: ExecTraceData['filters'] = {};
  if (params.pid) filters.pid = params.pid;
  if (params.name_pattern) filters.name_pattern = params.name_pattern;
  if (params.uid !== undefined) filters.uid = params.uid;
  if (params.include_failed) filters.failed_only = true;
...
async function runExecsnoop(
  ...
  let events: ExecEvent[] = parsed.executions.map((e) => ({
  ...
  const byParentSorted = Object.entries(byParent)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 20);
  const topByParent: Record<string, number> = {};
