...
export interface FdTraceData {
  ...
  leak_detection: {
    fd_growth_rate: number; // FDs per second
 ...
 */
async function getFdLimits(pid: number): Promise<{ soft: number; hard: number }> {
 ...
 */
function classifyFdType(target: string): FdInfo['type'] {
  if (target.startsWith('socket:')) return 'socket';
  if (target.startsWith('pipe:')) return 'pipe';
  if (target.startsWith('anon_inode:[eventfd]')) return 'eventfd';
  if (target.startsWith('anon_inode:[timerfd]')) return 'timerfd';
  if (target.startsWith('anon_inode:[signalfd]')) return 'signalfd';
  if (target.startsWith('anon_inode:[eventpoll]')) return 'epoll';
 ...
 */
async function getFdList(pid: number, maxFds: number): Promise<{ fds: FdInfo[]; total: number; byType: Record<string, number> }> {
...
export async function perfFdTrace(
  input: FdTraceRawInput
): Promise<PerfResponse<FdTraceData>> {
