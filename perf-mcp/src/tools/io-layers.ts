...
export interface IoLayersData {
  ...
  ratios: {
    vfs_to_block: number; // > 1 means caching is effective
    read_hit_rate: number; // Estimated read cache hit rate (%)
    write_coalesce_rate: number; // Estimated write coalescing (%)
  };
  analysis: {
...
/**
 * Parse /proc/diskstats for block I/O counts
 * Format: major minor name rd_ios rd_merge rd_sectors rd_ticks wr_ios wr_merge wr_sectors wr_ticks ...
 */
interface DiskStats {
  reads: number;
  writes: number;
...
function parseProcDiskstats(content: string): DiskStats {
  ...
  for (const line of lines) {
    ...
    if (name.match(/^(loop|ram|dm-)/)) continue;
    if (name.match(/\d+$/) && name.match(/^[hs]d/)) continue;
    ...
    if (!isNaN(rdIos)) totalReads += rdIos;
    if (!isNaN(wrIos)) totalWrites += wrIos;
    if (!isNaN(rdSectors)) totalReadSectors += rdSectors;
    if (!isNaN(wrSectors)) totalWriteSectors += wrSectors;
  }
...
export async function perfIoLayers(
  input: IoLayersRawInput = {}
): Promise<PerfResponse<IoLayersData>> {
  ...
  try {
    ...
    let cacheEffectiveness: 'excellent' | 'good' | 'fair' | 'poor';
