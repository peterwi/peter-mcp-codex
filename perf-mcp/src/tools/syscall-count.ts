...
import { executeBcc, type BccProgress } from '../lib/bcc-runtime.js';
import { getSyscountScript } from '../lib/bpftrace-fallbacks.js';
import { parseSyscount, parseSyscountWithLatency } from '../parse/bcc.js';
import { createFinding, createEvidence, type Finding, type Evidence } from '../lib/output-schema.js';
 ...
 */
function parseBpftraceSyscountOutput(output: string): SyscallEntry[] {
...
export async function perfSyscallCount(
  input: SyscallCountRawInput = {}
): Promise<PerfResponse<SyscallCountData>> {
  ...
  try {
    ...
    const result = await executeBcc({
      ...
      onProgress: (progress: BccProgress) => {
    ...
    let syscalls: SyscallEntry[];
    let totalLatencyUs: number | undefined;
    ...
    if (totalSyscalls > 1000000) {
      ...
      findings.push(createFinding(
        ...
        'warning',
        'High syscall rate',
        `System is making ${(rate / 1000).toFixed(1)}K syscalls/sec - may indicate busy polling or inefficient I/O`,
        'cpu',
        {
          confidence: 85,
    ...
    if (topSyscall) {
      ...
      if (topPercent > 50) {
        findings.push(createFinding(
          'dominant_syscall',
          'info',
          `${topSyscall.name} dominates`,
          `${topSyscall.name} accounts for ${topPercent.toFixed(1)}% of all syscalls`,
          'cpu',
          {
            confidence: 90,
    ...
    if (params.include_latency) {
      ...
      if (highLatency.length > 0) {
        for (const s of highLatency.slice(0, 3)) {
          findings.push(createFinding(
            'high_latency_syscall',
            'warning',
            `High latency: ${s.name}`,
            `${s.name} has average latency of ${(s.latency!.avg_us / 1000).toFixed(1)}ms`,
            'io',
            {
              confidence: 80,
