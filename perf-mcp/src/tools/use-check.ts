...
import type { IntervalSnapshotData } from './snapshot.js';
import type {
  PerfResponse,
  PerfUseCheckData,
  UseResourceMetrics,
 ...
 */
function isSnapshotData(data: PerfSnapshotData | IntervalSnapshotData): data is PerfSnapshotData {
  return !('mode' in data) || data.mode !== 'interval';
}

function determineStatus(value: number, thresholds: { warning: number; critical: number }): UseStatus {
...
function analyzeResource(
  ...
  utilizationThresholds: { warning: number; critical: number },
  utilizationDetail: string,
  saturationValue: number,
  saturationThresholds: { warning: number; critical: number },
  saturationDetail: string,
  errorCount: number,
  errorDetail: string
): UseResourceMetrics {
  return {
...
function analyzeCpu(snapshot: PerfSnapshotData, cpuCount: number): UseResourceMetrics {
  ...
  return analyzeResource(
    cpuUtil,
    USE_THRESHOLDS.cpu.utilization,
    `${cpuUtil.toFixed(1)}% busy (user=${utilization.user.toFixed(1)}%, sys=${utilization.system.toFixed(1)}%, iowait=${utilization.iowait.toFixed(1)}%)`,
    saturationValue,
...
function analyzeMemory(snapshot: PerfSnapshotData): UseResourceMetrics {
  ...
  const availablePercent =
    memory.total_bytes > 0 ? (memory.available_bytes / memory.total_bytes) * 100 : 100;
...
function analyzeDisk(snapshot: PerfSnapshotData): UseResourceMetrics {
  ...
  const mostQueued = devices.reduce((a, b) =>
    b.avg_queue_size > a.avg_queue_size ? b : a
  );
  ...
  const slowDevices = devices.filter(
    (d) => d.avg_wait_ms > USE_THRESHOLDS.disk.awaitMs.critical
  );
  ...
  return analyzeResource(
    utilizationValue,
    USE_THRESHOLDS.disk.utilization,
    `${mostUtilized.name}: ${utilizationValue.toFixed(1)}% busy`,
    saturationValue,
    USE_THRESHOLDS.disk.avgQueue,
    saturationDetail,
    errorCount,
    errorDetail
  );
...
function analyzeNetwork(snapshot: PerfSnapshotData): UseResourceMetrics {
  ...
  const retransmitRate =
    tcp.out_segs > 0 ? (tcp.retransmits / tcp.out_segs) * 100 : 0;
...
function generateSuspicions(resources: PerfUseCheckData['resources']): string[] {
  const suspicions: string[] = [];

  // Check each resource
  const checks: Array<{ name: string; metrics: UseResourceMetrics }> = [
...
function determineOverallStatus(
  resources: PerfUseCheckData['resources']
...
export async function perfUseCheck(): Promise<PerfResponse<PerfUseCheckData>> {
  ...
  try {
    ...
    const resources: PerfUseCheckData['resources'] = {
