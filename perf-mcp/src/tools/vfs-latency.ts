 ...
 */
function buildVfsLatencyScript(minLatencyMs: number, durationSec: number, pid?: number): string {
 ...
 */
function parseBpftraceVfsLatency(output: string): VfsOperation[] {
  ...
  for (const line of lines) {
    ...
    const operation = parts[3] as 'R' | 'W';
    const bytes = parseInt(parts[4], 10);
    const latency_ms = parseInt(parts[5], 10);

    if (isNaN(timestamp) || isNaN(pid) || isNaN(bytes) || isNaN(latency_ms)) continue;
...
export async function perfVfsLatency(
  input: VfsLatencyRawInput = {}
): Promise<PerfResponse<VfsLatencyData>> {
  ...
  try {
    ...
    let method: 'bcc_fileslower' | 'bpftrace' = 'bcc_fileslower';
    let operations: VfsOperation[] = [];
    ...
    // Aggregate by file
    const byFileMap = new Map<string, { count: number; totalLatency: number; totalBytes: number }>();
    ...
    const byProcessMap = new Map<string, { pid: number; count: number; totalLatency: number }>();
    for (const op of operations) {
